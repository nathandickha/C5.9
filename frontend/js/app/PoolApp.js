...
const concreteHatchTex = makeConcreteHatchTexture();
  ...
  focusCameraLikeStarterPreview(preset = null, { animate = false } = {}) {
    ...
    const halfVFov = THREE.MathUtils.degToRad((this.camera.fov || 90) * 0.5);
...
// PBR / Caustics integration should never hard-crash the app if a module fails
    ...
    // Defer the floating-point HDRI until the scene and base model are visible.
    const loadEnvironment = () => this.scene?.userData?.loadHDRIEnvironment?.().catch(() => {});
    requestAnimationFrame(() => requestAnimationFrame(() => {
      if ("requestIdleCallback" in window) {
        window.requestIdleCallback(loadEnvironment, { timeout: 1500 });
      } else {
        window.setTimeout(loadEnvironment, 200);
      }
    }));
  }
